...
4. Use CMake configure the build:
    ...
    On a MAC (M3) machine,  configure the build with:
    ```
...
You can run the main test suite (abytest) or any example. Since these are two-party protocols, each needs two terminals (one for each party).
For example, to run the Millionaire's Problem test:
...
Or to run the main ABY test suite:
